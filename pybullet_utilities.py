...
def print_image(view_matrix, proj_matrix, imgs_dir, image_num=None, extra_message=""):
    image_result = p.getCameraImage(1440, 1080, view_matrix, proj_matrix, renderer=p.ER_TINY_RENDERER, shadow=1)
...
def planar_center_of_rotation(angle, rotation_axis_sign, start_position, start_orientation, position, orientation):
    ...
    if angle == 0:
        cor_wc = np.matmul(rot, displacement)   #if angle is 0, then COR is at infinity, so give a COR that is directionally correct.
    else:
...
def make_pushing_scenarios_and_get_object_rotation_axes(original_scene_data, pushes_per_object_direction, cylinder_height_offset, push_distance, object_type_com_bounds_and_test_points,
    ...
    for object_index in np.arange(len(starting_data)):
        ...
        for i in np.arange(2):
            ...
            if mobile_object_types[object_index] == "mustard_bottle" or (mobile_object_types[object_index] == "bleach_cleanser" and i==0):
...
def get_com_bounds_for_object_type(object_type, crop_fraction_x, crop_fraction_y, crop_fraction_z):
    ...
    return {"com_bounds":(com_x_range, com_y_range, com_z_range),"full_bounds":(full_x_range, full_y_range, full_z_range)}
...
def write_PLY_files(dest_dir, view_matrix, proj_matrix, mobile_object_IDs):
    w, h, RGBA, depth, segmentation_mask = p.getCameraImage(1440, 1080, view_matrix, proj_matrix, renderer=p.ER_TINY_RENDERER)
